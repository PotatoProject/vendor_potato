...
function gerritpush()
{
    ...
    DEFAULT_BRANCH=hashbrown-release;
    PROJECT_PREFIX=;
    ref=for;
...
function updatefries() {
    ...
    git add -A
    git commit -m "prebuilts: Update Fries to v$(curl -sX GET https://raw.githubusercontent.com/PotatoProject/PotatoFries/hashbrown-release/pubspec.yaml | grep version | cut -d : -f 2 | xargs)"
    git push potato HEAD:hashbrown-release
    croot
}
...
function updateversion() {
    ...
    while [[ "$local" != "$remote" ]]; do
      sleep 5;
      git fetch potato hashbrown-release 2> /dev/null;
      local=$(git rev-list HEAD | head -1)
